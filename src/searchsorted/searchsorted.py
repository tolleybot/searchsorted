...
try:
    from searchsorted.cpu import searchsorted_cpu_wrapper
except ImportError:
...
try:
    from searchsorted.cuda import searchsorted_cuda_wrapper
except ImportError:
...
def searchsorted(a: torch.Tensor, v: torch.Tensor,
    ...
    if a.is_cuda and not SEARCHSORTED_GPU_AVAILABLE:
        raise Exception('searchsorted on CUDA device is asked, but it seems '
                        'that it is not available. Please install it')
    if not a.is_cuda and not SEARCHSORTED_CPU_AVAILABLE:
        raise Exception('searchsorted on CPU is not available. '
                        'Please install it.')
